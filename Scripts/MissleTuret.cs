...
public partial class MissleTuret : Weapon
{
    //TODO make behavioir unique
    ...
    public override void _Ready()
    {
        ...
        player = GetNode<AudioStreamPlayer2D>("Fire");
    ...
    {
        double val = 5 - (0.4 * shotspeed) - (0.1 * engineBonus);
        if (val <= 0)
        {
            val = 0.1;
        }
    ...
    public override void _Process(double delta)
    {
        ...
        if (controled == true)
        {
            ...
            if ((turret.Position.X - mousepos.X) < 0)
            {
                ...
                {
                    if (turret.Rotation < -0.5 || turret.Rotation > 0.5)
                    {
    ...
    private void Fire(Vector2 mousepos)
    {
        ...
        newbullet.startmoving(Target.GlobalPosition, mousepos, turret.GlobalRotation,spaceship.getRotationDir(),damage*2);
    ...
    private void _on_timer_timeout()
    {
        ...
        timer.WaitTime = (8 / (1 + engineBonus * shotspeed));
    }
